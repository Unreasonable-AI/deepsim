...
class Normalizer(tf.keras.layers.Layer):
  ...
  def __init__(self, size, max_accumulations=10**6, std_epsilon=1e-8,
    ...
    self._acc_count = tf.Variable(
      0, name=f'{self.name}/Variable',
      **var_kwargs
    )
    self._num_accumulations = tf.Variable(
      0, name=f'{self.name}/Variable_1',
      **var_kwargs
    )
    self._acc_sum = tf.Variable(
      tf.zeros(size, tf.float32), name=f'{self.name}/Variable_2',
      **var_kwargs
    )
    self._acc_sum_squared = tf.Variable(
      tf.zeros(size, tf.float32), name=f'{self.name}/Variable_3', 
      **var_kwargs
    )
  ...
  def call(self, batched_data, accumulate=True):
    ...
    # TODO: something about this workaround to avoid cond
    mask = tf.cast(
      tf.logical_and(
        accumulate,
        tf.less(self._num_accumulations, self._max_accumulations)
      ), tf.float32
    )
    self._accumulate(batched_data, mask)
  ...
  def _accumulate(self, batched_data, mask):
    ...
    self._acc_sum.assign_add(data_sum * mask),
    self._acc_sum_squared.assign_add(squared_data_sum * mask),
    self._acc_count.assign_add(count * mask),
    self._num_accumulations.assign_add(tf.convert_to_tensor(1, tf.float32) * mask)
