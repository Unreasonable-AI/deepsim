...
class Model(core_model.BaseModel):
  ...
  max_accumulations=10**6, name='Model'):
    ...
    self._output_normalizer = normalization.Normalizer(
        ...
        name=f'{self.name}/output_normalizer'
        )
    self._node_normalizer = normalization.Normalizer(
        size=3+common.NodeType.SIZE, 
        max_accumulations=max_accumulations,
        name=f'{self.name}/node_normalizer'
        )
    self._edge_normalizer = normalization.Normalizer(
        size=7, 
        max_accumulations=max_accumulations,
        name=f'{self.name}/edge_normalizer'
  ...
  def loss(self, inputs):
    ...
    # TODO: modify after adding option for batch size > 1
    # TODO: should error be average error per graph averaged over batch?
    # or average error across all graphs in batch?

    # Average error per graph averaged over batch
    # # total_loss = sum_{b=1}^batch_size(
    # # sum_{i=1}^{nodes_in_batch_b}(error_i * mask_i) / nodes_in_batch_b
    # # ) / batch_size
    # # = sum_{b=1}^batch_size graph_loss_b / batch_size
    ...
    # num_nodes = tf.reduce_sum(tf.cast(loss_mask, tf.int32))
    # all_reduce to get total number of nodes
    # num_nodes = self.strategy.reduce(tf.distribute.ReduceOp.SUM, num_nodes, axis=None)
    # # total_loss = sum_{i=1}^{total_nodes}(error_i * mask_i) / total_nodes
    # # = (sum_{b=1}^{batch_size}sum_{i=1}^{nodes_in_batch_b}(error_i * mask_i)) / total_nodes
    # # = sum_{b=1}^{batch_size} (sum_{i=1}^{nodes_in_batch_b}(error_i * mask_i)) / total_nodes)
    # # loss = sum_{i=1}^{nodes_in_batch_b}(error_i * mask_i)) / total_nodes
    # # later loss will be summed over all batches to get total_loss
    # loss = tf.reduce_sum(error[loss_mask]) / tf.cast(num_nodes, tf.float32)
